...
Redux and RTK for predicatable state management
Part 1:
* createSlice: for every action -> reducer function is written; generates actions and reducer
* useSelector() --> mapStateToProps
* useDispatch() --> mapDispatchToProps
* configureStore() instead of createStore()

Part 2:
createAsyncThunk() to use thunk middleware to handle async logic ==> built-in in RTK
prior: npm i redux react-redux redux-thunk
extraReducers: to handle actions generated by Thunk [ not ment for  handling actions from view]

Part 3:
createApi(): similar to createAsyncThunk but provides extra functionalites like caching, polling, pre-fetching, provides hooks for endpoints

Pre-fetching:
try Intersection Observer API: to implement infinite scroll

--------

Middleware: Thunk / Saga / ObservableJS ,,...

Redux Saga is based on ES6 generators
...
iter.next(); // {"name" : "A"}
condition ....
iter.next(); // starts from task 3 returns 200
condition ...
iter.next(); // task4, 5 and 6 --> true
